...
cdc_vax_total_first_doses <- round(cdc_vax_total_doses[2],2)
cdc_vax_total_second_doses <- round(cdc_vax_total_doses[4],2)
# total doses 
...
ggplot(map_data, aes(x = long, 
  ...
  scale_fill_gradient(low = "light blue", high = "blue", labels = scales::percent) +
...
ggplot(aes(x = date, y = daily_vaccinations)) +
  ...
  #snow storm
  annotate(geom="text", 
           x= daily_vax_doses$date[31], 
           y = 2.2, 
           label= "Nor'Easter", 
           color = "black",
           size = 3,
           font = "Georgia") +
  geom_segment(aes(x = daily_vax_doses$date[46], y = 0, xend= daily_vax_doses$date[46], yend = 2.35)) +
  
  #president's day
  annotate(geom="text", 
           x= daily_vax_doses$date[42], 
           y = 2.42, 
           label= "Presidents Day Holiday", 
           color = "black",
           size = 3,
           font = "Georgia") +
  geom_segment(aes(x = daily_vax_doses$date[33], y = 0, xend= daily_vax_doses$date[33], yend = 2.1)) +
  ...
  labs(x = NULL,
       ...
       subtitle = paste0("Seven day rolling average: ", seven_day_average_rate, " million total doses per day.\n",
                         "Last updated: ", format(as.Date(Sys.Date(),"%Y-%m-%d"), "%B %d, %Y")),
...
projected_vax_100 <- projected_vax_100 %>% distinct(date, .keep_all = TRUE)
...
#readr::write_csv(daily_vax_doses, "vax_time_series.csv")
...
daily_tweet <- paste0("🚨 Daily update ", format(as.Date(Sys.Date(),"%Y-%m-%d"), "%m/%d/%Y"), ":\n",
                      ...
                      "💉 Daily new doses: ", today_vax_doses, " million \\n",
                      "📈 7-day rolling average: ", seven_day_average_rate, " million doses/day\n",

                      "⚕️ % received at least one dose: ~", total_vax_perc, "% \n",

                      "📊 % to 70% herd immunity: ~", us_perc_to_immunity, "% \n",
                      "📅 Projected herd immunity date: ", herd_date, "\n",
...
```{r new herd graph, include=FALSE}
# daily cases
# daily_cases <- read_csv(url("https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-states.csv")) %>%
#   group_by(date) %>%
#   summarize(cases = sum(cases)) %>%
#   mutate(cases = 4.6*cases/1000000)
# 
# # daily vax
# daily_vaccines <- daily_vax_doses 
# 
# for(i in 1:nrow(daily_vaccines)){
#   daily_vaccines$total[i] <- sum(daily_vaccines$daily_vaccinations[1:i])
# }
# 
# daily_vaccines <- daily_vaccines %>% mutate(total = total/2) %>%
#   select(date,total) %>%
#   rename(vax_protected = total)
# 
# # protection
# 
# protection <- left_join(daily_cases, daily_vaccines, by = c("date"="date")) %>%
#   mutate(total_protected = cases + vax_protected)
# 
# total_protected <- max(protection$total_protected, na.rm = TRUE)
# 
# date <- Sys.Date() + 1
# 
# while(total_protected <= 297) {
#   
#   add_new_row <- data.frame(date, 
#                             4.6*seven_day_average_covid_case_rate + max(protection$cases, na.rm = TRUE),
#                             seven_day_average_rate + max(protection$vax_protected, na.rm = TRUE),
#                             4.6*seven_day_average_covid_case_rate + max(protection$cases, na.rm = TRUE) + seven_day_average_rate + max(protection$vax_protected, na.rm = TRUE))
#   
#   names(add_new_row) <- c("date","cases","vax_protected","total_protected")
# 
#   protection <- rbind(protection,add_new_row)
#   
#   total_protected <- max(protection$total_protected, na.rm = TRUE)
#   date <- date + 1
# }
# 
# protection <- protection %>% mutate(date = as.Date(date))
# protection %>%
#   ggplot(aes(x=date, y = total_protected)) +
#   geom_area(fill = "#19EEEC") +
#   geom_area(aes(x=date,y=cases), fill = "#FF364C") +
#   theme_tufte()+
#   
...
#        y = "People protected (in millions)",
#        fill = NULL,
#        title = paste0("An optimistic path to herd immunity"),
#        subtitle = paste0("At 90% threshold, projected date: ", format(protection$date[nrow(protection)],"%B %d, %Y"), "\n"),
#        caption = paste0("Last updated: ", format(as.Date(Sys.Date(),"%Y-%m-%d"), "%B %d, %Y"))) +
#   
#   theme(plot.caption = element_text(size = 12, face = "italic"),
#         plot.title = element_text(size=16, face = "bold"),
...
#         panel.grid.minor.y = element_line(colour = "grey90", size = 0.3))
# 
# ggsave(filename = "aggressive_herd_date.png", device = "png", width = 10, height = 6)
