...
## Segment Chart
```{r bar charts, echo = FALSE, warning = FALSE, message = FALSE, fig.align='center', fig.height= 14}
covid_summary <- daily_states %>%
  select(state, fips, cases, deaths, perc_pop_covid, perc_pop_death) %>%
...
ggplotly(ggplot(covid_summary, aes(x = state, 
                                   ...
                                   text = paste0("<b>State: </b>", state,"<br>",
                                                 "<b>% of pop. diagnosed: </b>", round(perc_pop_covid*100,2),"%"))) +
  
  geom_segment(aes(xend = state, yend=0)) +
  
  geom_point(size = 4, color = "black") +
  
# geom_text(aes(label = perc_pop_covid, nudge_x = -1)) +
